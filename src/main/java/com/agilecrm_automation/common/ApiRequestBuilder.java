...
/*
        Optional.ofNullable(headers)
                .ifPresentOrElse(var->  specification.headers(var),
                        ()-> System.out.println("headers are empty"));

        Optional.of(headers).ifPresent(var-> specification.headers(var));
        boolean fag = Optional.of(headers).isPresent();
        if(fag){
